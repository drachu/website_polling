...
class Info(db.Model):
    ...
    uzaleznienie_model = db.Column(
        db.Integer(), nullable=False, default=0)  # 1=tak, 0=nie
...
class Games(db.Model):
    ...
    pandemia_gra_wiecej = db.Column(
        db.String(length=10), nullable=False)  # tak-nie
    pandemia_czas = db.Column(db.String(length=10), nullable=False)  # tak-nie

    # Pozytywne odczucia //mieszane//
    pozytyw_ucieczka = db.Column(
        db.String(length=10), nullable=False)  # tak-nie
    pozytyw_samokontrola = db.Column(
        db.String(length=10), nullable=False)  # tak-nie
